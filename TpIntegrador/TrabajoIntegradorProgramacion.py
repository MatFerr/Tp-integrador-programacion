...
#Trae y lee los elementos almacenados en el archivo.csv
def traerDatos():
 datos.clear()
...
#Guarda los datos nuevos, sin sobreescribir los ya existentes
...
#Actualiza los datos del archivo.csv, sobreescribiendo los mismos
...
#Almacena solo los datos, cuyo 'pais' coincida con el string entregado como parametro
...
#Muestra los datos obtenidos desde un filtro ya aplicado
...
# OPCION 1: AÑADIR PAIS
   if(opcion == "1"):
    ...
    #Diferentes validaciones de los campos
        ...
        if(poblacionPais.isdigit() and int(poblacionPais) > 0):
           ...
           if(superficiePais.isdigit() and int(superficiePais) > 0):
              ...
              if(contienentePais.isalpha and contienentePais != "" and contienentePais.title() in continentes):
                 ...
                 print(f"Pais: {pais[0]} | Poblacion: {pais[1]} | Superficie: {pais[2]} | Continente: {pais[3]}")
...
# OPCION 2: ACTUALIZAR PAIS
      ...
      if(len(datosFiltrados) > 0):
         ...
         #Selecciona cual de todas las coincidencias con el nombre desea actualizar
         try: 
            ...
            #Se actualizan los datos del pais seleccionado
...
# OPCION 3: BUSCAR PAIS        
   ...
   #Si hay resultados, los muestra por pantalla
    if(len(datosFiltrados) > 0):
      print("==RESULTADOS==")
      for datoFiltrado in datosFiltrados:
         print(f"{i}) Pais: {datoFiltrado[0]} | Poblacion: {datoFiltrado[1]} | Superficie: {datoFiltrado[2]} | Continente : {datoFiltrado[3]}")
...
# OPCION 4: 'APLICAR FILTROS'
   elif(opcion == "4"):
      print("")
      datosFiltrados = []
      datos = traerDatos()
      print("==APLICAR FILTROS==")
      opcion = input("Por que deseas filtrar(numero de indice): 1) Continente 2) Poblacion 3) Superficie : ")

      #Si el usuario desea filtrar por 1) Continente
      if(opcion == "1"):
         print("==CONTINENTES==")
      ...
      #Si el usuario desea filtrar por 2) Poblacion            
      elif(opcion == "2"):
         print("==POBLACION==")
         rangoInicial = 0
         rangoFinal = 0
      ...
      #Si el usuario desea filtrar por 3) Superficie
      elif(opcion == "3"):
         print("==SUPERFICIE==")
         rangoInicial = 0
         rangoFinal = 0
...
#OPCION 5: APLICAR ORDENES
   elif(opcion == "5"):
      traerDatos()
      print("==ORDENAR PAISES==")
      opcion = input("A cual elemento del pais deseas aplicar el orden(nombre/poblacion/superficie/continente): ")

      #Si el usuario selecciona 'nombre'
      ...
      #Si el usuario selecciona 'continente'
      elif(opcion.lower() == "continente"):
         print("\n==ORDEN APLICADO: 'continentes'==")
         datosOrdenados = sorted(datos, key=lambda pais: pais[3])
         for datosOrdenado in datosOrdenados:
            print(f"Continente: {datosOrdenado[3]} | Poblacion: {datosOrdenado[1]} | Superficie: {datosOrdenado[2]} | Pais: {datosOrdenado[0]}")
      
      #Si el usuario selecciona 'poblacion'
      ...
      #Si el usuario selecciona 'superficie'
...
#OPCION 6: ESTADISTICAS GENERALES
   elif(opcion == "6"):
      traerDatos()
...
#LOGICA PARA LAS OBTENCION DE ESTADISTICAS  
      menorPoblacion = min(datos, key=lambda pais: int(pais[1]))
      mayorPoblacion = max(datos, key=lambda pais: int(pais[1]))    
      for i in range(len(datos)):
         promedioHab += int(datos[i][1])
         promedioSup += int(datos[i][2])
         if datos[i][3] in paisCont:
            paisCont[datos[i][3]] += 1
         else:
            paisCont[datos[i][3]] = 1

      #MUESTRA EN PANTALLA LOS ESTADISTICAS
      print("==MENOR POBLACION==")
      print(f"- '{menorPoblacion[0]}' con un total de {menorPoblacion[1]} habitantes\n")
      print("==MAYOR POBLACION==")
      print(f"- '{mayorPoblacion[0]}' con un total de {mayorPoblacion[1]} habitantes\n")
